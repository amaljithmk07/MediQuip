...
const RegistrationPage = () => {
  ...
  const inputClass = "w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all duration-200 shadow-sm";
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 to-medical-50 p-4 py-12">
      ...
      <div className="w-full max-w-2xl bg-white/70 backdrop-blur-xl p-8 md:p-12 rounded-3xl shadow-glass border border-slate-100 relative overflow-hidden animate-fade-in-up">
        ...
        <div className="flex justify-center items-center mb-10 relative z-10 max-w-md mx-auto">
          <div className={`w-8 h-8 flex items-center justify-center rounded-full font-bold text-sm ${step >= 1 ? 'bg-teal-600 text-white' : 'bg-slate-200 text-slate-500'} transition-colors duration-300`}>1</div>
          <div className={`flex-1 h-1 mx-2 rounded ${step >= 2 ? 'bg-teal-600' : 'bg-slate-200'} transition-colors duration-300`}></div>
          <div className={`w-8 h-8 flex items-center justify-center rounded-full font-bold text-sm ${step >= 2 ? 'bg-teal-600 text-white' : 'bg-slate-200 text-slate-500'} transition-colors duration-300`}>2</div>
          <div className={`flex-1 h-1 mx-2 rounded ${step >= 3 ? 'bg-teal-600' : 'bg-slate-200'} transition-colors duration-300`}></div>
          <div className={`w-8 h-8 flex items-center justify-center rounded-full font-bold text-sm ${step >= 3 ? 'bg-teal-600 text-white' : 'bg-slate-200 text-slate-500'} transition-colors duration-300`}>3</div>
        </div>
        ...
        <div className="relative z-10 max-w-md mx-auto">
          {step === 1 && (
            <div className="space-y-5 animate-fade-in-right">
              ...
              <div>
                ...
                <input type="password" name="password" value={letter.password || ''} onChange={letterHandler} placeholder="Create a strong password" className={inputClass} />
              </div>
              <button onClick={nextStep} className="w-full mt-6 bg-teal-600 hover:bg-teal-700 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-transform hover:-translate-y-0.5">
                Continue
              </button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-5 animate-fade-in-right">
              ...
              <div className="flex gap-4 mt-6">
                <button onClick={prevStep} className="w-1/3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 px-4 rounded-xl transition-colors">
                  Back
                </button>
                <button onClick={nextStep} className="w-2/3 bg-teal-600 hover:bg-teal-700 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-transform hover:-translate-y-0.5">
                  Continue
                </button>
              </div>
            </div>
          ...
          {step === 3 && (
            <div className="space-y-6 text-center animate-fade-in-right">
              ...
              <p className="text-slate-500">By creating an account, you agree to our Terms of Service and Privacy Policy.</p>
              
              <div className="flex gap-4 mt-8">
                <button onClick={prevStep} className="w-1/3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-3 px-4 rounded-xl transition-colors">
                  Back
                </button>
                <button onClick={createHandler} disabled={loading} className="w-2/3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-transform hover:-translate-y-0.5 disabled:opacity-70 disabled:cursor-not-allowed">
