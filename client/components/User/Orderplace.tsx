        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start animate-fade-in-up">
          ...
          <div className="w-full lg:w-3/5 space-y-6">
            ...
            <div className="bg-white rounded-3xl shadow-soft border border-slate-100 p-8 relative overflow-hidden">
              ...
              <div className="relative z-10 flex justify-between items-center mb-6">
                ...
                <Link onClick={addressChange} href="/user/address" className="text-sm font-bold text-teal-600 hover:text-teal-700 transition-colors bg-teal-50 px-4 py-2 rounded-xl">
                  Edit
                </Link>
              </div>
              ...
              {profile ? (
                <div className="relative z-10 space-y-4">
                  <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                    ...
                    <p className="text-slate-600 mb-3">{profile.address}, {profile.district}, {profile.state} - {profile.pin_code}</p>
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 text-sm text-slate-500">
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg> {profile.alternate_phone || 'N/A'}</span>
                      <span className="flex items-center gap-1.5"><svg className="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg> {profile.email || 'N/A'}</span>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="relative z-10 bg-amber-50 text-amber-800 p-5 rounded-2xl border border-amber-200 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm mt-1 opacity-80">Please add your delivery address before proceeding.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-emerald-50 rounded-3xl shadow-soft border border-emerald-100 p-6 flex gap-4 items-start">
              ...
              <div>
                <h3 className="text-lg font-bold text-emerald-900 mb-1">Medical Equipment Notice</h3>
                <p className="text-sm text-emerald-700 leading-relaxed">
                  The items requested are provided through donations. By proceeding, you acknowledge that you are requesting these for legitimate medical needs.
                </p>
              </div>
            </div>
          ...
          <div className="w-full lg:w-2/5">
            <div className="bg-white rounded-3xl shadow-glass border border-slate-100 p-8 sticky top-24">
              <h2 className="text-xl font-bold text-slate-800 mb-6 font-display">Request Summary</h2>
              
              <div className="space-y-4 mb-6 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                {cartitems && cartitems.length > 0 ? (
                  cartitems.map((item) => (
                    <div key={item._id} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                      <div className="flex flex-col">
                        <span className="font-semibold text-slate-700 line-clamp-1 text-sm">{item.name}</span>
              ...
              <div className="border-t border-slate-100 pt-6 mb-8">
                <label className="flex items-start gap-3 cursor-pointer group">
                  <div className="relative flex items-center justify-center mt-1">
                    ...
                    />
                    <div className={`w-5 h-5 rounded border ${agreed ? 'bg-teal-600 border-teal-600' : 'bg-white border-slate-300'} transition-colors flex items-center justify-center`}>
                  ...
                  <span className="text-sm text-slate-600 group-hover:text-slate-800 transition-colors select-none leading-relaxed">
                    I confirm that I take responsibility for these items and need them for medical purposes.
                  </span>
                </label>
              </div>
              ...
              <button 
                ...
                className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-4 px-4 rounded-xl shadow-lg transition-all duration-300 transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wider text-sm"
              >
