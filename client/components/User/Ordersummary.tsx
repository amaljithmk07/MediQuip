      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-2">Track the status of your requested medical equipment.</p>
          </div>
          <Link href="/user/viewproduct" className="text-teal-600 font-bold hover:text-teal-700 transition-colors hidden sm:block">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl shadow-soft border border-slate-100 p-16 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-slate-500 mb-8 max-w-md">You haven't requested any medical equipment yet.</p>
          ...
          <div className="space-y-8">
            {orders.map((order, index) => {
              ...
              return (
                <div key={order._id} className="bg-white rounded-3xl shadow-soft border border-slate-100 overflow-hidden animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
                  ...
                  <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">{order.category} &bull; Qty: {order.cart_qty}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full ${statusColor}`}>
                        {order.orderstatus || 'Pending'}
                      </span>
                      <Link href={`/volunteer/view-details/${order._id}`} className="text-sm font-semibold text-teal-600 hover:text-teal-700 bg-white border border-slate-200 hover:border-teal-200 px-3 py-1.5 rounded-lg transition-colors">
                        View Details
                      </Link>
                    </div>
                  </div>
                    ...
                    <div className="relative">
                      ...
                      <div className="relative flex flex-col sm:flex-row justify-between gap-8 sm:gap-0">
                        ...
                        <div className="flex sm:flex-col items-center gap-4 sm:gap-2 relative z-10 w-full sm:w-1/4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors ${currentStep >= 1 ? 'bg-teal-500 border-teal-500 text-white shadow-md' : 'bg-white border-slate-300 text-slate-400'}`}>
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>
                          </div>
                          <div className="sm:text-center">
                            <p className={`font-bold text-sm ${currentStep >= 1 ? 'text-slate-800' : 'text-slate-500'}`}>Requested</p>
                        ...
                        {/* Step 2: Approved */}
                        <div className="flex sm:flex-col items-center gap-4 sm:gap-2 relative z-10 w-full sm:w-1/4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors ${currentStep >= 2 ? 'bg-teal-500 border-teal-500 text-white shadow-md' : 'bg-white border-slate-300 text-slate-400'}`}>
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                          </div>
                          <div className="sm:text-center">
                            <p className={`font-bold text-sm ${currentStep >= 2 ? 'text-slate-800' : 'text-slate-500'}`}>Approved</p>
                        ...
                        {/* Step 3: Assigned */}
                        <div className="flex sm:flex-col items-center gap-4 sm:gap-2 relative z-10 w-full sm:w-1/4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors ${currentStep >= 3 ? 'bg-teal-500 border-teal-500 text-white shadow-md' : 'bg-white border-slate-300 text-slate-400'}`}>
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                          </div>
                          <div className="sm:text-center">
                            <p className={`font-bold text-sm ${currentStep >= 3 ? 'text-slate-800' : 'text-slate-500'}`}>Assigned</p>
                        ...
                        {/* Step 4: Delivered */}
                        <div className="flex sm:flex-col items-center gap-4 sm:gap-2 relative z-10 w-full sm:w-1/4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors ${currentStep >= 4 ? 'bg-emerald-500 border-emerald-500 text-white shadow-md' : 'bg-white border-slate-300 text-slate-400'}`}>
                            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
                          </div>
                          <div className="sm:text-center">
                            <p className={`font-bold text-sm ${currentStep >= 4 ? 'text-slate-800' : 'text-slate-500'}`}>Delivered</p>
